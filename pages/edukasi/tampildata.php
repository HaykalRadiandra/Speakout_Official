...
$text 	= "SELECT kodeedukasi,judul,ket,foto FROM edukasi WHERE onview=1 ";
...
while ($rec = mysqli_fetch_array($sql)) {	
	...
	echo " 
		<tr>
			...
			<td >{$rec['ket']}</td>";
	if(!empty($foto)){
		echo "<td class='text-center' width='250'><img class='img-fluid object-fit-cover' src='img/edukasi/$foto'></td>"; 
	}else{
		echo "<td class='text-center'></td>"; 
	}
	echo"
			<td class='text-center'>
				<a class='text-decoration-none' href='javascript:void(0)' onClick=\"edit('{$rec['kodeedukasi']}')\">Edit</a> &nbsp;&nbsp;| &nbsp;
				<a class='text-decoration-none' href='javascript:void(0)' onClick=\"del('{$rec['kodeedukasi']}')\">Hapus</a>
			</td>
		</tr>";	
