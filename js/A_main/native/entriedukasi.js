...
$(function() {
    ...
    $('#entriedukasi').on('click', function() {
        $('#vwdata').hide();	
        $('#endata').show();
        $('#vwfoto').hide();
    ...
    $("#btnconfirm").on('click', function () {
        ...
        let ket = $("#txtket").val();
        let gambar = upload(); // hasil bisa true, false, atau null
    ...
    $("#btnSave").on('click', function(){
        let kodeedukasi = $("#txtkodeedukasi").val();
        let judul = $("#txtjudul").val();
        let ket = $("#txtket").val();
        let foto = $("#txtfoto")[0].files[0];

        let formData = new FormData();
        formData.append("kodeedukasi", kodeedukasi);
...
function tampildata(page){
    ...
    $.ajax({
        type: "GET",
        url: "pages/edukasi/tampildata.php",
        data: {
            cari: cari,
            page: page
        },
...
function edit(kodeedukasi){
    ...
    $.ajax({
        type: "POST",
        url: "pages/edukasi/edit.php",
        data: "kodeedukasi="+kodeedukasi,
        dataType: "json",
        success: function(data){
            $("#txtkodeedukasi").val(data.kodeedukasi);
			$("#txtjudul").val(data.judul);
			$("#txtket").val(data.ket);
			if (data.foto) {
				$('#txtfotox').attr('src', 'img/edukasi/' + data.foto);
			}else {
				$('#vwfoto').hide();		
			}
        }
...
function del(kodeedukasi){
    $("#deleteModal").modal('toggle');
    $("#deleteModal").modal('show');
    $("#txtkodeedukasi").val(kodeedukasi);
}
...
$('#btnHapus').on('click', function(){
    ...
    $.ajax({
        type    : "POST",
        url     : "pages/edukasi/hapus.php",
        data    : {
            kodeedukasi : kodeedukasi
        },
        dataType: "json",
        success : function(data){
