...
$text = "SELECT 
		...
		LEFT JOIN (
			SELECT 
				...
				SUM(IF(onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS totalpelanggaran,  
				SUM(IF(STATUS = 1 AND onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS pelanggaranmenunggu,
				SUM(IF(STATUS = 2 AND onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS pelanggarandisetujui,
				SUM(IF(STATUS = 3 AND onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS pelanggaranditolak
			FROM pelanggaran 
		...
		LEFT JOIN (
			SELECT 
				DATE(tglentry) AS tglentry,  
				SUM(IF(onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS totalkehilangan,  
				SUM(IF(STATUS = 1 AND onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS hilangbelum,
				SUM(IF(STATUS = 2 AND onview = 1 AND kodepelapor='$kodeuser', 1, 0)) AS hilangsudah
			FROM kehilangan 
			GROUP BY DATE(tglentry)
		...
		LEFT JOIN (
			SELECT 
				DATE(h.tglentry) AS tglentry,  
				SUM(IF(h.onview = 1 AND i.kodeterlapor='$kodeuser', 1, 0)) AS totalhukuman,  
				SUM(IF(h.STATUS = 1 AND h.onview = 1 AND i.kodeterlapor='$kodeuser', 1, 0)) AS hukumanbelum,
				SUM(IF(h.STATUS = 2 AND h.onview = 1 AND i.kodeterlapor='$kodeuser', 1, 0)) AS hukumansudah
			FROM hukuman h
			LEFT JOIN pelanggaran i ON h.kodeaduan = i.kodeaduan
			GROUP BY DATE(h.tglentry)
		) c ON c.tglentry = xx.datefield

		WHERE xx.datefield BETWEEN '$tglawal' AND '$tglakhir'
";

$text .= "ORDER BY xx.datefield DESC";
